...
使用这个 Skill 时，将当前对话作为默认的主线程（main thread）和编排入口，由主线程按需派发聚焦任务给角色 Agent（role agents）、子 Agent（subagents）或用户可见的 Codex thread。主线程默认不承载执行过程；它负责理解意图、设定边界、决策、整合、冲突处理、验收和最终汇报。执行载体负责承载独立调研、实现、测试、评审、配置、排障、发布检查或执行层对话。

## 运行模式

- 默认让用户和主线程沟通需求、决策和最终验收，但不要把“只和主线程沟通”作为硬规则。
- 主线程默认少做执行活。配置、安装、排障、推送、长日志命令、UI 操作、凭据流程、批量修改和多轮试错，应优先放入执行载体；只有不会明显污染上下文的小范围只读检查、微小修改或强顺序动作，才由主线程直接完成。
...
- 主线程负责：目标澄清、约束管理、执行载体选择、任务派发、关键决策、证据整合、验收判断、状态摘要和最终汇报。
- 执行载体负责：命令执行、配置修改、文件编辑、测试运行、排障循环、日志阅读、UI 操作、凭据流程、调研展开和实现细节。
- 主线程记录 executive summary，不记录完整执行过程。除非决策需要，不把命令长输出、排障细节或中间尝试搬回主线程。
- 如果主线程发现自己正在积累大量执行层上下文，应停下来重新选择执行载体，并向用户说明迁移方式。
...
当任务很小、强顺序依赖明显，或委派成本高于收益时，可以由主线程直接完成，但这应是例外而不是默认。
...
- 主线程直接完成：intake、决策、整合、少量只读检查或微小低风险动作；一旦进入配置、排障、安装、推送、批量修改或多轮执行，应切出执行载体。
...
- 主线程保留 executive summary：目标、当前状态、关键决策、变更摘要、验证结论和剩余风险；执行过程的完整轨迹留在执行载体中。
...
2. Plan：判断执行载体，默认把执行层工作外移；说明是否需要委派、是否需要用户进入子 thread 对话，以及为什么这样选择。
3. Dispatch：把执行任务交给合适载体，说明输入、权限、边界和输出契约。
4. Execute：由执行载体实现、配置、排障或验证；主线程只在必要时做小范围协调。
