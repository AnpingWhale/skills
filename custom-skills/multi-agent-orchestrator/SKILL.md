...
使用这个 Skill 时，把复杂 Codex 工作作为 main-thread orchestration 来推进。主线程尽量避免亲自执行密集型任务，只保留目标澄清、载体选择、授权边界、结果整合和最终汇报；不承接主要审查结论，也不承接长执行或发布操作。执行载体负责命令密集或细节密集的工作，例如调研、实现、测试、review、修改文件、提交、推送、配置、安装、排障、发布检查、UI 操作、凭据流程和长日志。

目标不是尽量多用 Agent，也不是省总 token，而是做复杂度隔离、独立验证和主线程上下文治理，保护判断质量、减少上下文污染，并避免 self-validation。

## 适用阈值与成本预期

启用 multi-agent orchestration 的阈值：多文件、多轮、长日志、需要测试/review、涉及 git/发布/配置/凭据/安全/性能，或预计主线程会吞下大量执行细节。

总 token 在短任务中通常会上升；中等任务可能持平到略升；复杂长期任务即使总 token 增加，也能减少主线程上下文污染和 self-validation。

反过度编排：短、窄、低风险、预计 1-5 分钟内能完成的小任务，不强制多 Agent 编排；可使用一个轻量 role agent，或在没有合适委派载体且委派成本明显高于收益时，由主线程做最小只读核对。
...
- **completion notification 优先消费**：subagent 完成会产生 `completion notification`，但不会自动唤醒已经 final 的主线程；只有正在 `wait_agent`，或用户发来新消息/下一次主线程获得执行机会时，主线程才会消费通知。收到或发现完成通知后，第一优先级是消费通知、整合摘要、关闭 agent。
- **wait_agent 使用边界**：如果 subagent 结果是当前任务关键路径，主线程不得直接 final，必须使用一次较长 `wait_agent` 等待完成，然后继续整合、关闭 agent、汇报。只有不需要自动继续的后台任务，才允许主线程结束，依赖用户下次消息、可见 thread 主动回报，或 heartbeat 自动化回收。通知没有自动触发主线程响应是平台调度机制限制，不代表 subagent 没有回报。
- **heartbeat 边界**：heartbeat 是定时唤醒检查，不是事件驱动 callback；不要把它描述成 `completion notification` 的 completion callback。
...
- Main thread：intake、planning、carrier selection、authorization boundary、integration、final report。短、窄、低风险、预计 1-5 分钟内能完成的小任务，如果没有合适委派载体且委派成本明显高于收益，才可做最小只读核对；不得承担主要 review/audit 结论、写文件、长执行、提交、推送、安装配置或凭据流程。
...
- 默认不频繁轮询。但如果 subagent 结果是当前任务关键路径，主线程不得直接 final，必须使用一次较长 `wait_agent` 等待完成，然后继续整合、关闭 agent、汇报。
- 只有不需要自动继续的后台任务，才允许主线程结束，依赖用户下次消息、可见 thread 主动回报，或 heartbeat 自动化回收。
- `completion notification` 不会自动唤醒已经 final 的主线程；主线程下一次获得执行机会时，第一优先级是消费已完成通知、整合摘要并关闭 agent。
